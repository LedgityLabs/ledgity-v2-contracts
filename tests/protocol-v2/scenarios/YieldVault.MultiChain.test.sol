...
contract YieldVault_MultiChain_ScenarioTest is ScenarioEngine {
  ...
  /**
   * @notice Scenario: Stress test with rapid time jumps
   * @dev Tests vault stability with frequent time warps and actions
   */
  function test_scenario_rapidTimeJumps() public {
    ScenarioAction[] memory actions = new ScenarioAction[](25);
    uint256 idx = 0;
    ...
    // Rapid sequence of small time jumps with actions
    uint256[] memory timeJumps = new uint256[](8);
    timeJumps[0] = 1 days;
    timeJumps[1] = 3 days;
    timeJumps[2] = 7 days;
    timeJumps[3] = 2 days;
    ...
    for (uint256 i; i < timeJumps.length && idx < 25; i++) {
      // Time jump
      actions[idx++] = ScenarioAction({
        actionType: ActionType.TimeWarp,
        actor: testAccount1,
    ...
    executeScenario(actions, "Rapid Time Jumps");
  }
